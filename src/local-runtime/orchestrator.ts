...
export type LocalPipelineModel = "base" | { from: string };
export type LocalPipelineReference = { from: string };

export interface LocalPipelineStep {
  id: string;
  uses: "train" | "evaluate" | "compare";
  target: "local";
  with?: {
    model?: LocalPipelineModel;
    evaluator?: "behavior";
    before?: LocalPipelineReference;
    after?: LocalPipelineReference;
  };
}

/** Version-one ordered recipe executed by the local runtime. */
...
export interface LocalPipelineResult {
  ...
  outputs: Record<string, LocalPipelineOutput>;
  report?: RunReport;
  reportPath?: string;
  artifactDir: string;
}

export function canonicalLocalPipeline(): LocalPipeline {
  return {
    version: 1,
...
function referenceParts(reference: unknown): [string, string] | null {
  const value = typeof reference === "string"
    ? reference.replace(/^\$/, "")
    : reference && typeof reference === "object" && !Array.isArray(reference)
      ? (reference as { from?: unknown }).from
      : undefined;
  if (typeof value !== "string") return null;
  const match = /^([A-Za-z][A-Za-z0-9_-]*)\.(model|report)$/.exec(value);
  return match ? [match[1]!, match[2]!] : null;
}

/**
 * Validate every dependency before the runtime creates a store, claims an
 * artifact directory, or acquires a lock. The executor intentionally remains
 * an ordered list rather than a DAG scheduler.
 */
export function validateLocalPipeline(pipeline: LocalPipeline): LocalPipeline {
  if (!pipeline || pipeline.version !== 1 || !Array.isArray(pipeline.steps)) {
    throw new Error("Local pipeline must be a version 1 document with a steps array.");
  }
  const seen = new Map<string, LocalPipelineStep>();
  let hasTrainStep = false;
  for (const step of pipeline.steps) {
    if (!step || typeof step.id !== "string" || !/^[A-Za-z][A-Za-z0-9_-]*$/.test(step.id)) {
      throw new Error("Each local pipeline step requires an identifier starting with a letter.");
    }
    if (seen.has(step.id)) throw new Error(`Local pipeline step IDs must be unique: ${step.id}`);
    if (step.target !== "local") throw new Error(`Local pipeline step ${step.id} only supports target local.`);
    if (step.uses !== "train" && step.uses !== "evaluate" && step.uses !== "compare") {
      throw new Error(`Local pipeline step ${step.id} uses an unsupported component.`);
    }
    if (step.uses === "train") {
      if (hasTrainStep) {
        throw new Error("Local pipeline v1 supports at most one train step because model artifacts are run-scoped.");
      }
      hasTrainStep = true;
    }
    if (step.uses === "evaluate") {
      if (step.with?.evaluator !== "behavior") {
        throw new Error(`Local evaluate step ${step.id} requires the behavior evaluator.`);
      }
      if (step.with?.model !== "base") {
        const modelReference = referenceParts(step.with?.model);
        if (!modelReference || modelReference[0] === step.id || modelReference[1] !== "model") {
          throw new Error(`Local evaluate step ${step.id} requires model base or { from: '<earlier-train>.model' }.`);
        }
        const producer = seen.get(modelReference[0]);
        if (!producer) throw new Error(`Local evaluate step ${step.id} must reference an earlier train step.`);
        if (producer.uses !== "train") throw new Error(`Local evaluate step ${step.id} model reference must name a train step.`);
      }
    }
    if (step.uses === "compare") {
      for (const field of ["before", "after"] as const) {
        const reportReference = referenceParts(step.with?.[field]);
        if (!reportReference || reportReference[1] !== "report") {
          throw new Error(`Local compare step ${step.id} requires ${field}: { from: '<earlier-evaluate>.report' }.`);
        }
        const producer = seen.get(reportReference[0]);
        if (!producer) throw new Error(`Local compare step ${step.id} must reference an earlier evaluate step.`);
        if (producer.uses !== "evaluate") throw new Error(`Local compare step ${step.id} report reference must name an evaluate step.`);
      }
    }
  ...
  try {
    ...
    try {
      ...
      for (const step of pipeline.steps) {
        await throwIfCancelled(store, input.request);
        if (step.uses === "train") {
          const training = await runTrainStage({ prepared, config: input.config, store, reporter: input.reporter, runReporter });
          outputs[step.id] = { training, modelArtifactUri: training.model_artifact_uri };
          await refreshArtifactManifest(prepared);
          continue;
        }
        if (step.uses === "evaluate") {
          const isBase = step.with!.model === "base";
          const report = isBase
            ? await runBaselineStage({ prepared, config: input.config, store, reporter: input.reporter, runReporter })
            : await runCandidateStage({ prepared, config: input.config, store, reporter: input.reporter, runReporter });
          const reportPath = join(artifacts.runDir, "evaluations", `${step.id}.json`);
          await writeJsonAtomic(reportPath, { ...report, artifact_uri: fileUri(reportPath) });
          outputs[step.id] = { report: { ...report, artifact_uri: fileUri(reportPath) }, reportPath };
          await refreshArtifactManifest(prepared);
          continue;
        }
        const before = outputs[referenceParts(step.with!.before)![0]]?.report;
        const after = outputs[referenceParts(step.with!.after)![0]]?.report;
        if (!before || !after) throw new Error(`Local compare step ${step.id} is missing validated evaluation outputs.`);
        outputs[step.id] = { comparison: compareEvalReports(before, after) };
      }
      const compatibilityRecipe = pipeline.steps.length === 4
        && pipeline.steps.map((step) => `${step.id}:${step.uses}`).join(",")
          === "baseline:evaluate,train:train,candidate:evaluate,compare:compare";
      if (compatibilityRecipe) {
      ...
      await store.updateRun({
        ...
        message: "Pipeline steps completed; no full compatibility report was requested.",
        details: { completed_steps: pipeline.steps.map((step) => step.id) },
      });
      await input.reporter?.onEvent?.({
        stage: "stage_completed",
        status: "stage_completed",
        message: "Pipeline steps completed without a full compatibility report.",
      });
      return { request: input.request, pipeline, status: "stage_completed", outputs, artifactDir: artifacts.runDir };
    } catch (error) {
...
/** Backwards-compatible one-command entry point for the canonical v1 recipe. */
export async function runLocalFineTune(input: {
  ...
  if (!result.report || !result.reportPath) {
    throw new Error("Canonical local pipeline completed without a compatibility report.");
  }
  return {
